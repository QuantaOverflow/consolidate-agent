...
class EvidenceAgent:
    ...
    def embed_missing_sessions(self, session_xmls: dict[str, str], embedded_session_ids: set[str] | None = None) -> int:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(self._turn_store.embed_session, session_id, session_xmls[session_id])
                for session_id in missing_session_ids
            ]
            for future in as_completed(futures):
                added_turns += future.result()
        return added_turns
    ...
    def verify_batch(
        ...
        total_records = sum(len(v) for v in records_by_session.values())
        completed_records = 0
        admitted_records = 0
        with ThreadPoolExecutor(max_workers=self._workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    completed_records += len(records_by_session[session_id])
                    admitted_records += sum(1 for r in session_results if r.evidence_count > 0)
                    rate = admitted_records / completed_records if completed_records else 0
                    _progress(
                        f"verify progress={completed_records}/{total_records} "
                        f"admitted={admitted_records} admission_rate={rate:.0%}"
                    )
