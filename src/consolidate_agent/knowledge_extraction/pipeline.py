...
from consolidate_agent.context import ContextStats, ProcessedSession, SessionContextEngineer
...
def run_knowledge_extraction(
    ...
    try:
        ...
        for index, path in enumerate(paths, start=1):
            ...
            if session_id is not None and store.is_session_extracted(session_id):
                _skip_session(stats, "already_processed")
                continue

            extract_status = store.get_session_extract_status(session_id) if session_id else None
            existing_json = processed_index.sessions.get(session_id) if session_id else None
            if (
                extract_status in {None, ProcessedStatus.PENDING.value}
                and existing_json
                and existing_json.status == ProcessedStatus.PROCESSED
            ):
                _skip_session(stats, "already_processed")
                continue

            paths_to_process.append((path, session_id))

        def _preprocess_one(item: tuple[Path, str | None]) -> tuple[Path, str | None, ProcessedSession | None]:
            path, session_id = item
            cached_xml = store.get_processed_session(session_id) if session_id else None
            if cached_xml is not None:
                return path, session_id, _session_from_cached_xml(session_id, cached_xml)
            return path, session_id, engineer.process(path)

        with ThreadPoolExecutor(max_workers=MAX_PREPROCESS_WORKERS) as executor:
            preprocess_results = list(executor.map(_preprocess_one, paths_to_process))

        for path, _session_id, session in preprocess_results:
            if session is None:
            ...
            if store.get_processed_session(session.session_id) is None:
            ...
            if existing and existing.status == ProcessedStatus.PROCESSED and existing.normalized_hash == normalized_hash:
                _skip_session(stats, "already_processed")
                continue
            store.mark_session_extract_status(session.session_id, "processing", normalized_hash=normalized_hash)
        ...
        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_SESSIONS) as executor:
            ...
            for future in as_completed(future_to_session):
                ...
                try:
                    ...
                    store.mark_session_extract_status(session_id, "failed", error=_concise_error(exc))
                ...
                store.mark_session_extract_status(
                    session_id,
                    "processed",
                    normalized_hash=processed_index.sessions[session_id].normalized_hash,
                )
...
def _session_from_cached_xml(session_id: str, xml: str) -> ProcessedSession:
